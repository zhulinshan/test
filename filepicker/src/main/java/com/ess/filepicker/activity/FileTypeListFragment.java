...
public class FileTypeListFragment extends BaseFileFragment implements BaseQuickAdapter.OnItemClickListener,
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        int fileSize = getFileSize(item.getAbsolutePath());
        //选中某文件后，判断是否单选
        if (mIsSingle) {
        ...
        if (mAdapter.getData().get(position).isChecked()) {
            ...
            if ((max_size + fileSize) > (SelectOptions.getInstance().maxSize * 1012*1024)){
                Snackbar.make(mRecyclerView, "选择文件不能超过"+SelectOptions.getInstance().maxSize+"M", Snackbar.LENGTH_SHORT).show();
                return;
            }
